...
public class QuotedExtendedDefaultParser implements Parser {
    ...
    public ParsedLine parse(final String line, final int cursor, ParseContext context) {
        ...
        for (int i = 0; (line != null) && (i < line.length()); i++) {
            ...
            int blockQuoteMatched = matchBlockQuote(line,i);
            if(blockQuoteMatched > i){
                if(!blockQuoteStarted){
                    blockQuoteStarted = true;
//                    blockQuoteStart = blockQuoteMatched;
                    current.append(line.charAt(blockQuoteMatched));
                }else{
...
/*            if (quoteStart < 0 && isQuoteChar(line, i)) {
        ...
        return new QuotedExtendedDefaultParser.ExtendedArgumentList(line, words, wordIndex, wordCursor, cursor, openingQuote);
    }
    ...
    private int matchBlockQuote(final CharSequence buffer, final int pos){
        if(pos < 0){
            return pos;
        }

        for(int i = 0; i<blockQuote.length();i++){
            if(buffer.charAt(pos + i) != blockQuote.charAt(i)){
                return pos;
            }
        }
        return pos + blockQuote.length();
    }
